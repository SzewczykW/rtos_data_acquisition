...
//  <q>Use tickless idle
//  <i> Enable low power tickless mode to stop the periodic tick interrupt during idle
//  periods or <i> disable it to keep the tick interrupt running at all times. <i>
//  Default: 0
...
//  <i> Callback function vApplicationStackOverflowHook implementation is required when
//  stack checking is enabled. <i> Default: 0
#define configCHECK_FOR_STACK_OVERFLOW 2

//  <q>Use idle hook
//  <i> Enable callback function call on each idle task iteration.
//  <i> Callback function vApplicationIdleHook implementation is required when idle hook
//  is enabled. <i> Default: 0
#define configUSE_IDLE_HOOK 0

//  <q>Use tick hook
//  <i> Enable callback function call during each tick interrupt.
//  <i> Callback function vApplicationTickHook implementation is required when tick hook
//  is enabled. <i> Default: 0
#define configUSE_TICK_HOOK 0

//  <q>Use deamon task startup hook
//  <i> Enable callback function call when timer service starts.
//  <i> Callback function vApplicationDaemonTaskStartupHook implementation is required
//  when deamon task startup hook is enabled. <i> Default: 0
#define configUSE_DAEMON_TASK_STARTUP_HOOK 0

//  <q>Use malloc failed hook
//  <i> Enable callback function call when out of dynamic memory.
//  <i> Callback function vApplicationMallocFailedHook implementation is required when
//  malloc failed hook is enabled. <i> Default: 0
...
//  <i> Define event recording level bitmask for events generated from Event Groups
...
//  <i> Define event recording level bitmask for events generated from Stream Buffer
//  functions. <i> Default: 0x05
//    <0x00=>Off <0x01=>Errors <0x03=>Errors + API <0x05=>Errors + Operation
//    <0x07=>Errors + API + Operation <0x0F=>All
...
//  <i> When enabled RTOS objects can be created using RAM automatically allocated from
//  the FreeRTOS heap. <i> Default: 1
...
//  <i> Global heap buffer must be provided externally when using application allocated
//  heap. <i> The buffer must be declared as: uint8_t ucHeap[ configTOTAL_HEAP_SIZE ].
//  <i> Default: 0
#define configAPPLICATION_ALLOCATED_HEAP 0

//  <q>Use separate heap for stack allocation
//  <i> Enable or disable stack allocation for any task from a separate heap.
//  <i> Thread-safe implementation of pvPortMallocStack and vPortFreeStack is required
//  when using separate heap. <i> Default: 0
...
//  <i> Enable TrustZone when FreeRTOS runs on the Non-Secure side and calls functions
//  from the Secure side. <i> This setting is only relevant on ARMv8-M ports. <i>
//  Default: 1
#define configENABLE_TRUSTZONE 1

//  <o>Minimal secure stack size [words] <0-65535>
//  <i> Stack for idle task Secure side context in words.
//  <i> This setting is only relevant on ARMv8-M ports when TrustZone extension is
//  enabled. <i> Default: 128
#define configMINIMAL_SECURE_STACK_SIZE ((uint32_t)128)

//  <h>Interrupt Controller Configuration
//  <i> Configure Arm Generic Interrupt Controller (GIC) interrupt controller specific
//  settings. <i> Settings related to interrupt controller are relevant only on ARMv7-A
//  ports.
...
//   <i> Must be set to the offset from interrupt controller base address at which the
//   CPU interface starts. <i> Default: 0x1000
...
//  <i> Enable callback function call on each idle task iteration.
//  <i> Callback function vApplicationPassiveIdleHook implementation is required when
...
/* Defines that include FreeRTOS functions which implement CMSIS RTOS2 API. Do not
 * change! */
...
/* Maximum API call interrupt priority:
   - Interrupts assigned a priority at or below this priority can call interrupt safe
   FreeRTOS API function, and will nest.
   - Interrupts assigned a priority above this priority will not be effected by RTOS
   critical sections, and will nest, but cannot call any FreeRTOS API functions.
   - Use configMAX_SYSCALL_INTERRUPT_PRIORITY to set this define as they are essentially
   the same.
*/
